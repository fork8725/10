...
# Also import new models for dashboard update
from models import RawMaterialBatchRecord, MaterialBatchProcessRelation, EquipmentMaintenanceProductionRelation
...
# New dashboard target schemas
class RawMaterialBatchCreate(BaseModel):
    batchid: str = Field(min_length=1, max_length=50)
    materialid: str = Field(min_length=1, max_length=50)
    supplierid: str = Field(min_length=1, max_length=50)
    purchaseorderid: str = Field(min_length=1, max_length=50)
    batchquantity: float = Field(gt=0)
    storagetime: Optional[datetime] = None
    qualitycheckresult: Optional[Literal['合格','不合格','待检测']] = None
    inspectionreportid: Optional[str] = Field(default=None, max_length=50)
    storagelocation: str = Field(min_length=1, max_length=100)
    expirationdate: date
    usedquantity: float = Field(ge=0, default=0)
    batchstatus: Optional[Literal['在库','已用完','已冻结']] = None

class RawMaterialBatchRead(BaseModel):
    batchid: str
    materialid: str
    supplierid: str
    purchaseorderid: str
    ...
    qualitycheckresult: Optional[str]
    inspectionreportid: Optional[str]
    storagelocation: str
    expirationdate: date
    usedquantity: float
    remainingquantity: float
    batchstatus: Optional[str]
    class Config:
        from_attributes = True

class MaterialBatchProcessRelationCreate(BaseModel):
    relationid: str = Field(min_length=1, max_length=50)
    batchid: str = Field(min_length=1, max_length=50)
    processid: str = Field(min_length=1, max_length=50)
    usedquantity: float = Field(ge=0)
    usetime: Optional[datetime] = None
    operatorid: str = Field(min_length=1, max_length=50)
    recordstatus: Literal['正常','异常'] = '正常'

class MaterialBatchProcessRelationRead(BaseModel):
    relationid: str
    batchid: str
    processid: str
    usedquantity: float
...
class EquipmentMaintenanceProductionRelationCreate(BaseModel):
    relationid: str = Field(min_length=1, max_length=50)
    maintenanceid: str = Field(min_length=1, max_length=50)
    productionplanid: str = Field(min_length=1, max_length=50)
    impacthours: float = Field(ge=0, default=0)
    impactprocess: Optional[str] = Field(default=None, max_length=255)
    countermeasure: Optional[str] = Field(default=None, max_length=255)
    recoverytime: Optional[datetime] = None

class EquipmentMaintenanceProductionRelationRead(BaseModel):
    relationid: str
    maintenanceid: str
    productionplanid: str
    impacthours: float
    impactprocess: Optional[str]
    countermeasure: Optional[str]
    recoverytime: Optional[datetime]
    class Config:
        from_attributes = True
...
# New CRUD for RawMaterialBatchRecord
@app.post("/raw-material-batches", response_model=RawMaterialBatchRead, status_code=201)
def create_raw_material_batch(payload: RawMaterialBatchCreate, db: Session = Depends(get_db), _: User = Depends(require_admin)):
    if db.query(RawMaterialBatchRecord).filter(RawMaterialBatchRecord.batchid == payload.batchid).first():
        raise HTTPException(status_code=400, detail="batchid already exists")
    now = payload.storagetime or datetime.now(timezone.utc)
    remaining = float(payload.batchquantity) - float(payload.usedquantity)
    if remaining < 0:
        raise HTTPException(status_code=400, detail="usedquantity exceeds batchquantity")
    rec = RawMaterialBatchRecord(
...
@app.get("/raw-material-batches", response_model=List[RawMaterialBatchRead])
def list_raw_material_batches(db: Session = Depends(get_db)):
    return db.query(RawMaterialBatchRecord).order_by(RawMaterialBatchRecord.storagetime.desc()).all()

@app.delete("/raw-material-batches/{batchid}", status_code=204)
def delete_raw_material_batch(batchid: str, db: Session = Depends(get_db), _: User = Depends(require_admin)):
    rec = db.query(RawMaterialBatchRecord).filter(RawMaterialBatchRecord.batchid == batchid).first()
    if not rec:
        raise HTTPException(status_code=404, detail="RawMaterialBatchRecord not found")
    db.delete(rec)
    db.commit()
    return None

# New CRUD for MaterialBatchProcessRelation
@app.post("/material-batch-process-relations", response_model=MaterialBatchProcessRelationRead, status_code=201)
def create_material_batch_process_relation(payload: MaterialBatchProcessRelationCreate, db: Session = Depends(get_db), _: User = Depends(require_admin)):
    if db.query(MaterialBatchProcessRelation).filter(MaterialBatchProcessRelation.relationid == payload.relationid).first():
        raise HTTPException(status_code=400, detail="relationid already exists")
    # batch must exist
    if not db.query(RawMaterialBatchRecord).filter(RawMaterialBatchRecord.batchid == payload.batchid).first():
        raise HTTPException(status_code=400, detail="batchid not found")
    usetime = payload.usetime or datetime.now(timezone.utc)
    rel = MaterialBatchProcessRelation(
        relationid=payload.relationid,
        batchid=payload.batchid,
...
@app.get("/material-batch-process-relations", response_model=List[MaterialBatchProcessRelationRead])
def list_material_batch_process_relations(db: Session = Depends(get_db)):
    return db.query(MaterialBatchProcessRelation).order_by(MaterialBatchProcessRelation.usetime.desc()).all()

@app.delete("/material-batch-process-relations/{relationid}", status_code=204)
def delete_material_batch_process_relation(relationid: str, db: Session = Depends(get_db), _: User = Depends(require_admin)):
    rel = db.query(MaterialBatchProcessRelation).filter(MaterialBatchProcessRelation.relationid == relationid).first()
    if not rel:
        raise HTTPException(status_code=404, detail="MaterialBatchProcessRelation not found")
    db.delete(rel)
    db.commit()
    return None

# New CRUD for EquipmentMaintenanceProductionRelation
@app.post("/equipment-maintenance-production-relations", response_model=EquipmentMaintenanceProductionRelationRead, status_code=201)
def create_equipment_maintenance_production_relation(payload: EquipmentMaintenanceProductionRelationCreate, db: Session = Depends(get_db), _: User = Depends(require_admin)):
    if db.query(EquipmentMaintenanceProductionRelation).filter(EquipmentMaintenanceProductionRelation.relationid == payload.relationid).first():
        raise HTTPException(status_code=400, detail="relationid already exists")
    rel = EquipmentMaintenanceProductionRelation(
        relationid=payload.relationid,
        maintenanceid=payload.maintenanceid,
        productionplanid=payload.productionplanid,
        impacthours=payload.impacthours,
        impactprocess=payload.impactprocess,
...
@app.get("/equipment-maintenance-production-relations", response_model=List[EquipmentMaintenanceProductionRelationRead])
def list_equipment_maintenance_production_relations(db: Session = Depends(get_db)):
    return db.query(EquipmentMaintenanceProductionRelation).order_by(EquipmentMaintenanceProductionRelation.relationid.desc()).all()

@app.delete("/equipment-maintenance-production-relations/{relationid}", status_code=204)
def delete_equipment_maintenance_production_relation(relationid: str, db: Session = Depends(get_db), _: User = Depends(require_admin)):
    rel = db.query(EquipmentMaintenanceProductionRelation).filter(EquipmentMaintenanceProductionRelation.relationid == relationid).first()
    if not rel:
        raise HTTPException(status_code=404, detail="EquipmentMaintenanceProductionRelation not found")
    db.delete(rel)
    db.commit()
    return None
