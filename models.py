...
from sqlalchemy import Column, Integer, String, Text, TIMESTAMP, JSON, DECIMAL, CheckConstraint, ForeignKey, Date, Float
...
# New: RawMaterialBatchRecord based on design doc 4.2.1
class RawMaterialBatchRecord(Base):
    __tablename__ = "rawmaterialbatchrecord"
    batchid = Column(String(50), primary_key=True, nullable=False, unique=True)
    materialid = Column(String(50), nullable=False)
    supplierid = Column(String(50), nullable=False)
    purchaseorderid = Column(String(50), nullable=False)
    batchquantity = Column(DECIMAL(18, 2), nullable=False)
    storagetime = Column(TIMESTAMP, nullable=False)
    qualitycheckresult = Column(String(20), nullable=True)
    inspectionreportid = Column(String(50), nullable=True)
    storagelocation = Column(String(100), nullable=False)
    expirationdate = Column(Date, nullable=False)
    usedquantity = Column(DECIMAL(18, 2), nullable=False, default=0)
    remainingquantity = Column(DECIMAL(18, 2), nullable=False, default=0)
    batchstatus = Column(String(20), nullable=True)

    __table_args__ = (
        CheckConstraint("batchquantity > 0", name="chk_batchquantity_pos"),
        CheckConstraint("usedquantity >= 0", name="chk_usedquantity_nonneg"),
        CheckConstraint("batchstatus IN ('在库','已用完','已冻结')", name="chk_batchstatus"),
        CheckConstraint("qualitycheckresult IN ('合格','不合格','待检测')", name="chk_qualitycheckresult"),
    )

# New: MaterialBatchProcessRelation based on design doc 4.2.2 (1)
class MaterialBatchProcessRelation(Base):
    __tablename__ = "materialbatchprocessrelation"
    relationid = Column(String(50), primary_key=True, nullable=False, unique=True)
    batchid = Column(String(50), ForeignKey("rawmaterialbatchrecord.batchid"), nullable=False)
    processid = Column(String(50), nullable=False)
    usedquantity = Column(DECIMAL(18, 2), nullable=False)
    usetime = Column(TIMESTAMP, nullable=False)
    operatorid = Column(String(50), nullable=False)
    recordstatus = Column(String(20), nullable=False, default='正常')

    __table_args__ = (
        CheckConstraint("usedquantity >= 0", name="chk_mbpr_usedquantity_nonneg"),
        CheckConstraint("recordstatus IN ('正常','异常')", name="chk_recordstatus"),
    )

# New: EquipmentMaintenanceProductionRelation based on design doc 4.2.2 (2)
class EquipmentMaintenanceProductionRelation(Base):
    __tablename__ = "equipmentmaintenanceproductionrelation"
    relationid = Column(String(50), primary_key=True, nullable=False, unique=True)
    maintenanceid = Column(String(50), nullable=False)
    productionplanid = Column(String(50), nullable=False)
    impacthours = Column(Float, nullable=False, default=0.0)
    impactprocess = Column(String(255), nullable=True)
    countermeasure = Column(String(255), nullable=True)
    recoverytime = Column(TIMESTAMP, nullable=True)

    __table_args__ = (
        CheckConstraint("impacthours >= 0", name="chk_impacthours_nonneg"),
    )
